...
| 归档任务调度 | 归档任务编排、状态转移、执行链路 | ✅ 支持基于 BackgroundTask 的任务编排、排队与后台 HostedService 执行；分区切换归档功能已上线，含预检/补齐/执行全流程；BCP/BulkCopy 跨库归档已完成测试验证 |
| 运维控制 | 后台命令队列、执行托管、运行监控 | ✅ 命令队列与 HostedService 框架落地，执行日志可回放（Markdown 格式），任务监控看板已完成；心跳监控与僵尸任务恢复机制已完善 |
| 归档执行引擎 | 执行模板、脚本渲染、Dapper 执行封装 | ✅ 模板仓库与脚本生成器上线，分区切换执行器已完成；BCP/BulkCopy 执行器已实现并测试通过，支持跨服务器归档 |
...
### 数据归档功能完成 🎉 (2025-10-30 ~ 2025-11-13)
...
#### BCP/BulkCopy 跨库归档 (2025-11-13) ✨
- **执行引擎**：实现 `BcpExecutor` 和 `SqlBulkCopyExecutor`，支持跨服务器数据归档。
- **性能优化**：分区表归档采用 SWITCH + BCP/BulkCopy 策略，生产表影响降低 99%（< 1秒锁定时间）。
- **连接架构**：支持自定义目标服务器配置，包括 Windows 集成认证和 SQL Server 认证，密码自动加密存储。
- **进度跟踪**：实时进度回调，批次大小可配置，支持超时控制和错误处理。
- **DbContext 并发修复**：Repository 层统一使用 `AsNoTracking()`，心跳更新与执行流程完全隔离，杜绝并发冲突。
- **跨服务器验证**：✅ BCP 跨库归档测试通过（源服务器 → 临时表 SWITCH → BCP 导出/导入 → 目标服务器）。
- **临时表管理**：自动创建/清理临时表，失败场景保留临时表便于排查和重试，支持孤儿临时表自动清理。
...
### 待完善事项 📋
- **归档配置管理 UI**：完善归档配置的创建、编辑、启用/禁用界面（后端 API 已完成）。
- **用户操作手册**：编写分区管理、BCP/BulkCopy 归档操作等用户手册与 FAQ。
- **监控告警增强**：任务监控看板已完成，待补充失败告警通知和自动重试策略。
- **性能优化**：大表 BulkCopy 内存占用优化，BCP 临时文件清理策略优化。
...
- ✅ **Milestone D** (2025-11-13)：BCP/BulkCopy 跨库归档实现与测试验证 ✨
- 📋 **Milestone E** (计划中)：归档配置管理 UI + 定时调度功能
