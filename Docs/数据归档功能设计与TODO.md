...
- **环境准备**：
  ...
  - 如需，自动更新统计、重建索引。
- **超时策略**：长时间运行的 DDL 默认设置为无限制超时，确保对超大表执行时不会因命令超时中断。
...
- **安全增强（2025-11-04）**: 
  - 残留数据检测改为**仅检测+阻断**模式，不自动清空
  - 检测到数据时提供手动清空脚本，要求用户确认后执行
  - 防止误删除业务数据，提高归档操作安全性

### 4.5 已知限制与解决方案（2025-11-04）

#### 4.5.1 分区方案对齐问题
**问题描述**：
- 工具创建的分区能正常归档（分区函数、方案、索引均对齐）
- 已有分区（非工具创建）可能存在：
  - 分区方案名称不一致
  - 索引未对齐到分区方案
  - 约束定义差异
...
**影响范围**：
- 预检阶段会识别为 Blocker 并阻止执行
- 用户需手动调整后才能归档

**解决方案**：
1. **推荐方式**：使用工具创建分区配置，确保规范一致
2. **已有分区处理**：
   - 运行预检接口获取详细差异报告
   - 根据 Blocker 提示手动调整（索引重建、约束同步等）
   - 或使用自动补齐功能修复部分问题（需谨慎）
...
> **里程碑 C 已完成** ✅：数据归档（分区切换）功能全面交付，支持单分区和批量归档，具备完整的预检、自动补齐和执行能力。已进行初步测试并增强安全控制。
> 
> **下一步优先事项**：
> 1. 编写用户操作手册（含截图和最佳实践，重点说明已有分区对齐问题）
> 2. 整理 FAQ 文档（常见错误及解决方案，包括索引未对齐、约束不匹配等）
> 3. 编写已有分区诊断与修复指南
> 4. 性能优化（大批量分区归档场景）
> 5. BCP/BulkCopy 方案技术调研