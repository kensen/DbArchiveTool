...
1. **基础设施与 API**
   - 提供表/列/目标库/文件目录读取接口。
   - 新增分区配置与分区值创建接口，更新应用层 DTO。
   - 扩展 EF/领域模型以存储文件组模式、目标表等信息。
2. **应用层与领域逻辑**
   - 新建 `PartitionConfigurationAppService` 与相关命令处理，含校验。
   - 在域模型中实现存放模式、记住文件路径、目标表命名规则等约束。
3. **前端（Blazor）**
   - 在 `Partitions.razor` 集成分步向导组件，完成数据绑定与校验。
   - 实现分区值添加器（手动/批量）及状态管理。
   - 处理文件路径、最小最大值显示、错误提示。
4. **测试与文档**
   - Application/Domain 单元测试覆盖校验逻辑。
   - Integration 测试验证元数据接口。
   - 更新用户文档与 README 功能规划。
