...
- **后端服务**：
  ...
  - ✅ 任务创建与入队（`PartitionExecutionTask`）
...
#### 拆分分区功能
- ⏳ 后端服务：`SplitPartitionCommand` 执行器（基础代码已完成，待集成测试）
- ⏳ 前端组件：`PartitionSplitWizard` 三步向导
  - Step 1: 选择待拆分分区（表格展示 + 行数/文件组信息）
  - Step 2: 设置新边界值（实时预览拆分后范围）
  - Step 3: 执行计划预览与确认
- ⏳ 预检逻辑：索引对齐、文件组容量、备份确认
- ⏳ 集成测试：成功拆分、边界越界失败、锁冲突处理

#### 合并分区功能
- ⏳ 后端服务：`MergePartitionCommand` 执行器（基础代码已完成，待集成测试）
- ⏳ 前端组件：`PartitionMergeWizard` 三步向导
  - Step 1: 选择相邻分区对
  - Step 2: 确认合并结果（数据量、目标文件组）
  - Step 3: 执行计划预览
- ⏳ 校验逻辑：分区相邻性检查、数据量警告
- ⏳ 集成测试：成功合并、非相邻分区拒绝
...
| **Milestone B** | 前端添加分区值子控件上线 | ✅ 完成 | 2025-10-17 |
| **Milestone C** | 数据归档（分区切换）闭环 | 🔄 进行中 | 预计 2025-10-20 |
| **Milestone D** | 拆分/合并分区功能上线 | ⏳ 待开始 | 预计 2025-10-25 |
...
**P0（高优先级 - 本周完成）**：
1. **数据归档向导前端集成**（最关键，对齐旧工具核心能力）
   - 完成 `PartitionArchiveWizard` 四步向导 UI
   - 集成已完成的后端 API（inspect + switch）
   - 端到端测试（成功切换、结构不匹配、索引不齐等场景）
2. **任务调度平台 UI 完善**
   - 操作类型列展示与筛选
   - 归档任务详情页（目标库/表信息）
   - 日志分类渲染
