...
- **校验重点**：
  - 两个分区必须相邻且无待执行任务
  - 数据量过大时提前警告并建议离峰执行
  - 阻止合并行数为 0 的分区，避免误删历史归档数据
  - 确保源服务器上没有活跃的分区切换/拆分任务持有锁
- **安全措施**：执行前自动暂停相关调度任务、执行后更新源服务器统计并恢复。
- **当前差距**：同源同库且已存在“分区配置草稿”时归档流程已验证通过；若缺少草稿，尚需定义默认配置装载、参数采集与前端交互方案。
...
#### 数据归档功能（已完成 ✅ 2025-10-30）
- ✅ 前端组件：`PartitionArchiveWizard` 四步向导
  - ✅ Step 1: 归档方案选择（分区切换/BCP/BulkCopy）
  - ✅ Step 2: 方案参数配置（源分区、目标库/表、配置关联）
  - ✅ Step 3: 预检与风险评估（Blocker/AutoFix/Warning 展示）
  - ✅ Step 4: 执行确认（Markdown 脚本预览、任务提交）
- ✅ **分区切换方案**（100% 完成）：
  - ✅ `PartitionSwitchInspectionService`（结构/索引/约束/权限/锁检查）
  - ✅ `IPartitionSwitchAppService`（Inspect + AutoFix + Execute）
  - ✅ `PartitionSwitchAutoFixExecutor`（6 种自动补齐步骤）
  - ✅ API 路由：`POST /archive/switch/{inspect,autofix,execute}`
  - ✅ 前端集成与向导 UI（四步向导完整实现）
  - ✅ 端到端测试（单分区/批量归档验证通过）
  - ✅ 批量归档支持（多分区选择、依次执行、进度展示）
  - ✅ 后台任务执行（8 阶段流程、Markdown 日志）
  - ✅ 索引对齐检测与自动修复
- ⏳ **BCP/BulkCopy 方案**（占位状态，计划中）：
  - ✅ 接口定义与占位响应
  - ✅ UI 展示"规划中"提示
  - ⏳ 需求收集与技术调研（待启动）
...
| **Milestone C** | 数据归档（分区切换）闭环 | ✅ 完成 | 2025-10-30 |
| **Milestone D** | 拆分/合并分区功能上线 | ✅ 完成 | 2025-10-27 |
| **Milestone E** | BCP/BulkCopy 占位与调研 | ⏳ 计划中 | 待定 |

**🎉 核心功能全部交付完成！** 分区管理平台已具备完整的边界值管理（添加/拆分/合并）和数据归档（分区切换）能力。

### 9.5 核心功能特性总结（2025-10-30）

#### 已实现的核心能力 ✅

**1. 分区边界管理**（Milestone A & B）
- ✅ **添加分区值**：
  - 单值手动添加（支持 datetime/int/bigint/varchar 等类型）
  - 批量生成（日期范围按天/月/年递增，数值序列生成）
  - 边界值预览与管理（Tag 展示、单个删除/批量清空）
  - 文件组选择与自动分配
  - 前端完整校验（Range Left/Right 顺序、重复值、格式）
- ✅ **分区拆分**：
  - 2 步向导（设置新边界 → 确认执行）
  - 从主页面预选分区
  - 文件组选择（继承或指定）
...
- ✅ **分区合并**：
  - 2 步向导（选择边界 → 确认执行）
  - 智能合并预览（显示左/右边界、合并后范围）
  - 最小边界数验证（至少 2 个边界）
  - 后台任务执行与日志记录

**2. 数据归档（分区切换）**（Milestone C）
- ✅ **四步向导流程**：
  - Step 1: 方案选择（分区切换启用，BCP/BulkCopy 占位）
  - Step 2: 参数配置（源分区、目标库/表、配置关联、创建临时表选项）
  - Step 3: 预检结果（Blocker/AutoFix/Warning 分类展示、自动补齐勾选）
  - Step 4: 执行确认（Markdown 脚本预览、风险确认、任务提交）
- ✅ **预检服务**（`PartitionSwitchInspectionService`）：
  - 结构对齐检查（列定义、主键、IDENTITY、ROW_GUID）
  - 索引对齐检查（聚簇/非聚簇、包含列、填充因子、压缩）
  - 约束检查（CHECK、DEFAULT、UNIQUE、外键）
  - 分区配置检查（函数、方案、边界值、Range 方向）
  - 权限检查（ALTER、CONTROL 权限）
  - 锁冲突检查（`sys.dm_tran_locks`）
  - 文件组检查（可写状态、剩余空间）
  - 触发器拦截（阻止带触发器的表切换）
- ✅ **自动补齐执行器**（`PartitionSwitchAutoFixExecutor`）：
  - CreateTargetTable：创建目标表（含分区列、主键、列属性）
  - SyncPartitionObjects：同步分区函数与分区方案
  - SyncIndexes：批量创建索引（保持属性一致）
  - SyncConstraints：同步 CHECK/DEFAULT 约束
  - CleanupResidualData：检测残留数据并提示手动清空
  - RefreshStatistics：刷新统计信息
  - 失败回滚机制（栈式补偿设计）
- ✅ **后台任务执行**（`BackgroundTaskProcessor.ExecuteArchiveSwitchAsync`）：
  - 8 阶段流程：解析快照 → 加载数据源 → 验证分区 → 边界检查 → 索引对齐 → 预检确认 → 执行切换 → 任务完成
  - 索引对齐自动修复（重建未对齐的索引到分区方案）
  - 事务控制（失败自动回滚）
  - Markdown 格式日志（SQL 代码块高亮）
  - 进度跟踪（0.1 → 0.2 → ... → 1.0）
- ✅ **批量归档模式**：
  - 支持多分区选择（从主页面传递多个分区号）
  - 依次执行（每次间隔 500ms）
  - 实时进度展示（成功/失败计数、进度条）
  - 结果汇总表格（分区号、状态、消息、任务 ID）
- ✅ **配置关联功能**：
  - 可选择已保存的分区配置
  - 自动填充目标表信息
  - 配置不存在时使用源表信息
...
**3. 任务调度与监控**
- ✅ `BackgroundTask` 扩展（`OperationType` 字段区分操作类型）
- ✅ Markdown 日志渲染（`MarkdownViewer` 组件）
- ✅ SQL 代码块高亮（`markdown.css` 样式）
- ✅ 任务详情页优化（显示操作类型、归档目标、执行阶段）
- ✅ 批量执行进度展示（成功/失败统计）

**4. 技术亮点**
- 事务控制：所有 DDL 操作在事务内执行，失败自动回滚
- 连接池管理：统一使用 `IDbConnectionFactory` 管理数据库连接
- 异步执行：后台任务队列处理，避免阻塞用户界面
- 快照机制：保存执行参数和 SQL 脚本，便于审计和排查
- 细粒度日志：每个阶段记录详细日志，包含 SQL、执行时间、错误堆栈
- 索引对齐修复：自动检测并重建未对齐的索引
- 批量操作支持：多分区归档、实时进度展示
...
1. ✅ **分区边界管理核心功能**（已于 2025-10-27 全部完成）
   ...
   - ✅ 端到端测试验证通过
2. ✅ **数据归档功能（分区切换方案）**（已于 2025-10-30 全部完成）
   - ✅ 四步向导（方案选择 → 参数配置 → 预检结果 → 执行确认）
   - ✅ 预检服务完整（结构/索引/约束/权限/锁检查）
   - ✅ 自动补齐执行器（6 种修复步骤、失败回滚）
   - ✅ 后台任务执行（8 阶段流程、Markdown 日志）
   - ✅ 批量归档支持（多分区依次执行、进度展示）
   - ✅ 端到端测试验证通过
